...
public class Miner implements Runnable{
    ...
    public boolean doProofOfWork(Block oBlock) {
		...
		### ADD CODE HERE ###
		#####################
		First create a string that has as many zeros in it as oBlock.getDifficulty()
        equals. Use a while loop here that keeps looping while the length of this string you’re creating is
        ...
        while(true){
            if (bAbortPoW){
                bAbortPoW = false;
                System.out.println("[miner] Aborted mining block, " +
                        "probably because another confirmed block received.");
                return false;
            } else{
                oBlock.setNonce("" + iNonce);
                oBlock.setHash(oBlock.computeHash());
                if(oBlock.getHash().startsWith(zeros)){
                    return true;
                }
                iNonce++;
            }
